...
export async function getAuthors() {
  ...
  for (const post of posts) {
    if (post.data.author) {
      const posts = authors.get(post.data.author) || []
      posts.push(post)
      authors.set(post.data.author, posts)
    }
  }
...
export function getPathFromAuthors(
  author: string,
  authors_map: { name: string, path: string }[],
) {
  const mappingPath = authors_map.find(l => l.name === author)
  return mappingPath ? mappingPath.path : author
}
